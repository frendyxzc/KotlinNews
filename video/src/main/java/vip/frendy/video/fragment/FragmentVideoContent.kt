 ...
 * 新闻内容页
 */
class FragmentVideoContent : Fragment(), View.OnClickListener {
    private var rootView: View? = null
    private var uid: String = "0"
    private val mFragments: ArrayList<FragmentVideoList> = ArrayList()
    private val mTitles: ArrayList<String> = ArrayList()

    companion object {
        fun getInstance(bundle: Bundle): FragmentVideoContent {
            val fragment = FragmentVideoContent()
    ...
    override fun onCreateView(inflater: LayoutInflater?, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        if (rootView == null) {
            rootView = inflater?.inflate(R.layout.fragment_video_content, container, false)
            initData(arguments)
            initView()
        }
    ...
    private fun getChannel(uid: String) = doAsync {
        val channelList = Request(context).getChannelList(uid, 1)
        uiThread {
            for (channel in channelList) {
                val args: Bundle = Bundle()
                args.putString("uid", uid)
                args.putString("cid", channel.id)
                mFragments.add(FragmentVideoList.getInstance(args))
